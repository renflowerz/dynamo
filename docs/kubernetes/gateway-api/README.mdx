...
deployment-level settings such as replicas and resources to fit gateway traffic volume, and set the
`DYN_*` environment values to match the worker backend.
...
The Dynamo EPP runs the full Dynamo KV-aware router natively and is configured through `DYN_*`
environment variables. It implements the GAIE
...
`ext_proc` interface and does not use a plugin or scheduling-profile config. Disaggregated versus
aggregated routing is determined automatically from the worker types registered through Dynamo
discovery.
...
The worker component role label comes from the Dynamo
[ComponentType](../api-reference.md#componenttype) field.
...
Update them when the worker backend changes.
