...
# Router-only on-ramp (raw vLLM + Dynamo EPP + in-process selector)
...
```text
                          ┌───────────── Dynamo EPP (router-only, in-process selector) ────────────┐
...
4. On each request the EPP tokenizes locally (for routing only), selects a
   worker constrained to the currently-Ready pods (booking its load via
   select-and-reserve), and returns the worker's endpoint and `dp_rank` to Envoy
   as routing headers. The worker re-tokenizes the forwarded request.
...
| `DYN_EPP_SELECTOR_THREADS` | KV indexer thread pool | optional; default 4 |
| `DYN_EPP_PEER_SERVICE` | The EPP's OWN `Service`; the EPP watches its EndpointSlices to find sibling replicas and sync active load with them. Set = replicated; unset = single local replica | optional; unset by default |
| `DYN_EPP_PEER_SYNC_PORT` | ZMQ replica-sync port each EPP replica binds and dials peers on (all replicas must agree) | optional; default 9092 |
...
2. It watches its **own** `Service`'s EndpointSlices (the `DYN_EPP_PEER_SERVICE`
   ...
   on the sync port; the Service does not need to expose it.
